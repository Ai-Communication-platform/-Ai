...
class MyHandler(FileSystemEventHandler):
    def on_created(self, event):
        # 이 함수는 새 파일/디렉토리가 생성될 때마다 호출됩니다.
        if not event.is_directory:
            # 새로운 파일에 대해 함수 실행
            path = event.src_path
            print("current_size : " , os.path.getsize(path))
...
def process_new_file(path):
    ...
    # 여기에 새 파일이 생성될 때 실행할 코드를 추가합니다.
    try:
        with open(path, 'rb') as audio_file:
            content = audio_file.read()
            print(type(content))
            # content 변수에 파일의 내용이 저장됩니다.
            # 이후 content를 처리하는 코드를 추가하세요.
    ...
    # content가 None이 아니라면, 이어지는 작업을 수행합니다.
    if content is not None:
        # 파일 내용을 사용하여 작업을 수행합니다.
        audio = speech.RecognitionAudio(content=content)
        # 이후 필요한 처리를 계속합니다.
    else:
        # content가 None이면 오류 메시지를 출력하거나 다른 작업을 수행합니다.
        print("Failed to read the audio file content.")
        exit()
